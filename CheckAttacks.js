...
function checkDistanceForAttack(fighter, opponent) {
    const intersectX =
        Math.abs(opponent.x - fighter.currentMove.damageX)
        < (opponent.width + fighter.currentMove.damageWidth) / 2
    ...
    const intersectY =
        Math.abs(opponentCentralY - fighterCentralDamageY)
        < (opponent.height + fighter.currentMove.damageHeight) / 2
