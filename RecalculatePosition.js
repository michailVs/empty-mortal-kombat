...
export function recalculatePosition(fighter1, fighter2) {
...
function recalculateFighterPosition(fighter, opponent) {
    ...
    if (!fighter.isJumping() && opponent.isJumping() || fighter.isJumping() && !opponent.isJumping()) return

    if (!fighter.isMoving()) return

    const haveXCollision = 
        Math.abs(opponent.x - fighter.x)
        < (opponent.width + fighter.width) / 2
    const opponentCentralY = opponent.y - opponent.height / 2
    const fighterCentralY = fighter.y - fighter.height / 2
    const haveYCollision =
        Math.abs(opponentCentralY - fighterCentralY)
        < (opponent.height + fighter.height) / 2
    ...
    const collisionWidrh =
        (opponent.width + fighter.width) / 2
        - Math.abs(opponent.x - fighter.x)
    if (fighter.orientation === ORIENTATIONS.LEFT) {
        fighter.x -= collisionWidrh / 2
        opponent.x += collisionWidrh / 2
    } else {
        fighter.x += collisionWidrh / 2
        opponent.x -= collisionWidrh / 2
    }
